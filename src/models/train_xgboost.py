...
def prepare_data(df, target_col='Close', n_lags=5):
    """
    Prepare features for XGBoost.
    Uses technical indicators and creates target variable (next day price).
    """
    # Drop rows with NaN
    df = df.dropna()
    ...
    # Select features (exclude target and original OHLCV that have leakage)
...
def train_xgboost_model(ticker, data_file, mlflow_tracking_uri):
    """
    ...
    """
    print("Training XGBoost model for " + ticker)
    ...
    with mlflow.start_run(run_name="XGBoost_" + ticker):
        ...
        for feat, importance in top_10_features:
...
if __name__ == "__main__":
    ...
    if data_file.exists():
        train_xgboost_model("AAPL", str(data_file), mlflow_uri)
    else:
